...
def minimax(rows, is_max):
    ...
    # -1 means AI lost, 1 means AI won
    if sum(rows) == 0:
    ...
    # Return the best score: max for AI's turn (maximizing score), min for player's turn
    return max(scores) if is_max else min(scores)

# Find the best move by maximizing AI's chances to win or draw
def best_move(rows):
...
def play_nim():
    ...
    while sum(rows) > 0:  # Keep playing as long as there are stones left
        ...
        if not (0 <= i < len(rows) and 1 <= take <= rows[i]):
            print("Invalid move!")  # Invalid if row number or stones to take are out of bounds
            continue
        ...
        # AI's turn: calculate the best move using minimax
        ...
        if sum(rows) == 0:
            print("AI wins!"); break  # End the game when there are no stones left
