...
def astar(graph, heuristics, start, goal):
    ...
    while open_set:
        ...
        for neighbor, weight in graph[current]:  # <-- No .items() now
...
# Read graph from CSV
graph_df = pd.read_csv("exp8_graph.csv")

# Create graph as a defaultdict of list of tuples (neighbor, weight)
graph = defaultdict(list)
for _, row in graph_df.iterrows():
    graph[row["source"]].append((row["target"], row["weight"]))

# Read heuristics from CSV
heuristics_df = pd.read_csv("exp8_heuristics.csv")
