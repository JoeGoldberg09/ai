...
def initialize_weights(input_size, hidden1_size, hidden2_size, output_size):
    np.random.seed(42)
    w1 = np.random.randn(input_size, hidden1_size) * np.sqrt(1.0 / input_size)
    b1 = np.zeros((1, hidden1_size))
    w2 = np.random.randn(hidden1_size, hidden2_size) * np.sqrt(1.0 / hidden1_size)
...
def forward_pass(X, w1, b1, w2, b2, w3, b3):
    ...
    a2 = tanh(z2)
    z3 = np.dot(a2, w3) + b3
    output = sigmoid(z3)  # Output layer: Sigmoid
